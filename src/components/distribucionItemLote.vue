...
async function getInfo() {
    try {
        ...
        const response = await storeDisItemLote.getById(idDistribucion.value)
...
const goLoteFicha = (idDistribucionPresupuesto) => {
  router.push(`/distribucion-item-lote/${idDistribucionPresupuesto}`);
};
</script>
    ...
    <main style=" width: 100%; display: flex; justify-content: center;">
        ...
        <q-table :rows="rows" :columns="columns" row-key="name" :loading="loadingTable" loading-label="Cargando..."
            ...
            <template v-slot:body-cell-opciones="props">
                ...
                <button class="btn-go" @click="goLoteFicha(props.row._id)">Fichas <i class="fa-solid fa-arrow-right"></i></button>
                </q-td>
            </template>
        </q-table>
    </main>
