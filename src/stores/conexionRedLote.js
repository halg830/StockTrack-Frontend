...
export const useStoreConexRedLote = defineStore(modelo, () => {
  ...
  const getPorRed = async (idRed) => {
    try {
      insertarToken()
      const response = await axios.get(`${modelo}/buscarPorRed/${idRed}`);
      console.log(response);
      return response.data;
    } catch (error) {
      console.log(error);
  ...
  return { conexionesRedLote, getAll, agregar, editar, activar, inactivar, getById, getPorLote , getPorRed};
});
