...
const modelo = "detSalida";

export const useStoreDetalleSalida = defineStore(modelo, () => {
  ...
  const getPorSalida =async(idSalida)=>{
    try {
      insertarToken()
      const response = await axios.get(`${modelo}/getPorSalida/${idSalida}`)
      console.log(response);
      return response.data
    } catch (error) {
      console.log(error);
  ...
  return { getAll, getPorSalida, agregar, detallesSalidas}
})
